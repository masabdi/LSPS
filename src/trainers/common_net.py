...
class GaussianVAE(nn.Module):
  ...
  def sample(self, x):
    mu = self.en_mu(x)
    sd = self.softplus(self.en_sigma(x))
    noise = Variable(torch.randn(mu.size(0), mu.size(1))).cuda(x.data.get_device())
...
class LeakyINSResNeXtBlock(nn.Module):
  def __init__(self, inplanes, planes, k = 2, cardinality=8, dropout=0.0):
    ...
    model = []
    model += [nn.Conv2d(inplanes, k*inplanes, kernel_size=1, stride=1, padding=0)]
    model += [nn.InstanceNorm2d(k*inplanes)]
    model += [nn.LeakyReLU(inplace=True)]
    model += [nn.Conv2d(k*inplanes, k*inplanes, kernel_size=3, stride=1, padding=1, groups=cardinality)]
    model += [nn.InstanceNorm2d(k*inplanes)]
    model += [nn.LeakyReLU(inplace=True)]
    model += [nn.Conv2d(k*inplanes, planes, kernel_size=1, stride=1, padding=0)]
