...
class dataset_hand_ICVL(data.Dataset):
    def __init__(self, specs):
        ...
        self.Seq = self.di.loadSequence(
            subset, ['0'], rng=self.rng, shuffle=True, docom=docom)
        ...
        self.hd = HandDetector(img, abs(self.di.fx),
                               abs(self.di.fy), importer=self.di)
        self.num = len(self.Seq.data)
        print(' data loaded with %d samples' % self.num)

    def sample_poses(self):
        train_cube = np.asarray(
            [self.Seq.config['cube']]*self.num, dtype='float32')
        train_com = np.asarray([d.com for d in self.Seq.data], dtype='float32')
        train_gt3D = np.asarray(
            [d.gt3Dcrop for d in self.Seq.data], dtype='float32')
...
class dataset_hand_ICVL_test(dataset_hand_ICVL):
    ...
    def __getitem__(self, i):
        ...
        if i < self.len_seq1:
            ...
            gt3D = np.asarray(
                self.Seq2.data[i-self.len_seq1].gt3Dcrop, dtype='float32')
            img = np.asarray(
                self.Seq2.data[i-self.len_seq1].dpt.copy(), 'float32')
...
class dataset_hand_NYU(data.Dataset):
    def __init__(self, specs):
        ...
        self.di = NYUImporter(
            root, refineNet=None, allJoints=True, com_idx=com_idx, cacheDir='../../cache/')
        ...
        self.Seq = self.di.loadSequence(
            subset, rng=self.rng, shuffle=True, docom=docom)
...
class dataset_hand_NYU_test(dataset_hand_NYU):
    def __init__(self, specs):
        ...
        self.rng = np.random.RandomState(seed)
        self.di = NYUImporter(root, refineNet=None,
                              allJoints=True, cacheDir='../../cache/')

        self.Seq = self.di.loadSequence(
            subset, shuffle=False, rng=self.rng, docom=docom)
        self.num = len(self.Seq.data)
        print(' data loaded with %d samples' % self.num)
